#' Fixed version of prelim.norm from package norm
#'
#'This function fixes the way that mdp is generated so that x can have more than 32 columns
#' @param x A data matrix.
#' @return cf norm::prelim.norm
prelim.norm2 = function (x){
  ...
  if (sum(is.na(x)) < length(x)) {
    ...
    x <- norm::.na.to.snglcode(x, mvcode)
    tmp <- .Fortran("ctrsc", x, n, p, numeric(p), numeric(p),
                    mvcode, PACKAGE = "norm")
    x <- tmp[[1]]
    xbar <- tmp[[4]]
    sdv <- tmp[[5]]
    x <- norm::.code.to.na(x, mvcode)
  }
  if (sum(is.na(x)) == length(x)) {
    xbar <- rep(0, p)
    sdv <- rep(1, p)
